...
if (searchString.indexOf("desktop")>0) forceDesktop=true;
if (searchString.indexOf("mobile")>0) forceMobile=true;

//**************************
// Mercedes-AMG code
// If device is mobile, send them to the mobile 
// version of the site, if not, allow them to 
// load the default page.

//alert("Redirect:"+DetectMobileQuick()+","+DetectTierTablet()+","+forceDesktop);

//if( ( DetectMobileQuick() || DetectTierTablet() ) && !forceDesktop || forceMobile){

if(  DetectMobileQuick()  && !forceDesktop || forceMobile){

    var startOfUrl = "http://m.mercedes-amg.com/";
    var oldUrl = document.location.href;
    var storedHash = window.location.hash;

    oldUrl = oldUrl.replace(storedHash, '');
    //alert('oldUrl: '+ oldUrl)
    storedHash = storedHash.replace('#/', '#');

    var slashPos = oldUrl.lastIndexOf("/");
    var endOfUrl = oldUrl.substring(slashPos+1);


    if (typeof branch =="undefined"){
        endOfUrl = "index.php";
    }
    else if (branch=="vehicles"){
        var vehicleSections = Array("interior","powertrain","specifications","dna","media");
        for (var s=0; s<vehicleSections.length; s++){
            var vs = vehicleSections[s];
            if (section==vs) {
                var pos = endOfUrl.indexOf(".");
                endOfUrl = endOfUrl.substring(0,pos)+"_"+vs+endOfUrl.substring(pos);
            }
        }
    }
    else if (branch=="engineering" && page=="creation"){
        var engineSections = Array("20l4c","55lv8","55lv8biturbo","60lv12","63lv8","63lv8sls");
        for (var s=0; s<engineSections.length; s++){
            var es = engineSections[s];
            if (section==es) {
                var pos = endOfUrl.indexOf(".");
                endOfUrl = endOfUrl.substring(0,pos-10)+"_"+es+endOfUrl.substring(pos);
            }
        }
    }
    else if (branch=="latest"){
        var latestSections = Array("news","video","wallpapers");
        var latestSections2 = Array("news","videos","wallpapers");
        for (var s=0; s<latestSections.length; s++){
            var ls = latestSections[s];
            var ls2 = latestSections2[s];
            if (section==ls) {
                var pos = endOfUrl.indexOf(".");
                endOfUrl = endOfUrl.substring(0,pos)+"_"+ls2+endOfUrl.substring(pos);
            }
        }
    }

    var newUrl = startOfUrl + endOfUrl + storedHash;
